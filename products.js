import bestsellers from "./bestsellers.json" assert { type: "json" };
import newarrivals from "./newarrivals.json" assert { type: "json" };
...
bestsellers.map((el, index) => {
  ...
  h4.innerText = `₹`+`${el.price.toLocaleString()}`+`.00`;
...
newarrivals.map((el, index) => {
  ...
  document.querySelector("#newArrivals").append(div);
});
